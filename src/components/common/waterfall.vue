    ...
    <div :class="{'waterfall-wrap': true, row: row}" ref="container">
        <template v-if="row">
            <div v-for="(item, index) of list"
                 :key="index"
                 class="row-item">
                <img :src="item.imgUrl" :style="{height: `${height}px`}">
            </div>
        ...
        if (!this.row) {
          ...
          this.$nextTick(() => {
            ...
            for (let i = 0; i < boxes.length; i++) {
              this.setElementStyle(boxes[i], this.list[i], i, boxes.length)
            }
          })
        }
      },
      setElementStyle (element, img, index, length) {
        // 计算出图片实际在项目中显示的高
        const w = this.$refs.container.offsetWidth / 4
        const h = ((w - 6) / img.width) * img.height + 6 + 80 + 120
        if (index < this.column) {
          ...
          if (index === length - 1) {
            document.querySelector('.waterfall-wrap').style.height = min.value + 600 + 'px'
          }
        }
